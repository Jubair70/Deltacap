...
from onadata.apps.usermodule.models import UserModuleProfile, UserPasswordHistory,Organizations,OrganizationDataAccess,MenuItem, Institution
...
class UserProfileForm(forms.ModelForm):
    ...
    organisation_name = forms.ModelChoiceField(label='Organisation Name',required=True,queryset=Organizations.objects.all(),empty_label="Select an Organization",widget=forms.HiddenInput())
    ...
    class Meta:
        ...
        fields = ('admin','organisation_name','country')
        #fields = ('admin','organisation_name','country','institution')
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_retype_new_password(self):
        ...
        try:
            ...
            raise forms.ValidationError('Entered username is incorrect')
        # get list of last 24 password\
        #count_unusable_recent_password = 24  // Previously Used
        count_unusable_recent_password = 1
...
class UserRoleMapForm(forms.ModelForm):
    ...
    role = forms.ModelChoiceField(queryset=OrganizationRole.objects.filter(organization_id = 36), empty_label="(Nothing)")
    class Meta:
        model = UserRoleMap
        fields = ('user', 'role')
        # widgets = {
...
class UserRoleMapfForm(forms.Form):
    ...
    role = forms.ModelChoiceField(queryset=OrganizationRole.objects.filter(organization_id = 36), empty_label=None)
